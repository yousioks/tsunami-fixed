document.addEventListener('DOMContentLoaded', async function () {
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(error.detail || 'Session check failed') // <-- Добавь error.detail
		}
	...
	bonusForm.addEventListener('submit', async function (e) {
		...
		try {
			...
			if (data.success) {
				...
				// Исправлено: теперь выводит реальную причину ошибки от сервера
				alert(data.detail || 'Error applying bonus. Please try again.')
			}
		} catch (error) {
	...
	checkoutButton.addEventListener('click', async function () {
		...
		try {
			...
			if (data.success) {
				...
				alert(
					'Error processing order. Please check your balance and try again.'
				)
			}
		} catch (error) {
